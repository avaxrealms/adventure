...
contract adventure_crafting is ERC721Enumerable {
    ...
    function get_token_uri_goods(uint _item) public view returns (string memory output) {
        item memory _data = items[_item];
        {
            ...
            output = string(abi.encodePacked(output, "crafted at ", toString(_data.crafted), '</text><text x="250" y="300" class="title">Adventure</text><line x1="175" y1="305" x2="310" y2="305" style="stroke:#ba3e4a;stroke-width:1"/></svg>'));
        }
    ...
    function get_token_uri_armor(uint _item) public view returns (string memory output) {
        item memory _data = items[_item];
        {
            ...
            output = string(abi.encodePacked(output, "cost ", toString(_cost/1e18), "gp", '</text><text x="10" y="80" class="base">'));
            output = string(abi.encodePacked(output, "weight ", toString(_weight), "lb", '</text><text x="10" y="100" class="base">'));
            output = string(abi.encodePacked(output, "proficiency ", armor.get_proficiency_by_id(_proficiency), '</text><text x="10" y="120" class="base">'));
    ...
    function get_token_uri_weapon(uint _item) public view returns (string memory output) {
        item memory _data = items[_item];
        {
            ...
            output = string(abi.encodePacked(output, "proficiency ", weapons.get_proficiency_by_id(_weapon.proficiency), '</text><text x="10" y="120" class="base">'));
            output = string(abi.encodePacked(output, "encumbrance ", weapons.get_encumbrance_by_id(_weapon.encumbrance), '</text><text x="10" y="140" class="base">'));
            output = string(abi.encodePacked(output, "damage 1d", toString(_weapon.damage), " ", weapons.get_damage_type_by_id(_weapon.damage_type), '</text><text x="10" y="160" class="base">'));
