...
contract adventure_attributes is AccessControl {
    ...
    function _increase_base(uint _summoner) internal {
        ...
        require(abilities_by_level(adv.level(_summoner)) - _points_spent > 0);
    ...
    function tokenURI(uint256 _summoner) public view returns (string memory) {
        ...
        parts[1] = string(abi.encodePacked("strength", " ", toString(_attr.strength), '</text><text x="25" y="40" class="base">'));

        parts[2] = string(abi.encodePacked("dexterity", " ", toString(_attr.dexterity), '</text><text x="25" y="60" class="base">'));

        parts[3] = string(abi.encodePacked("constitution", " ", toString(_attr.constitution), '</text><text x="25" y="80" class="base">'));

        parts[4] = string(abi.encodePacked("intelligence", " ", toString(_attr.intelligence),  '</text><text x="25" y="100" class="base">'));

        parts[5] = string(abi.encodePacked("wisdom", " ", toString(_attr.wisdom), '</text><text x="25" y="120" class="base">'));

        parts[6] = string(abi.encodePacked("charisma", " ", toString(_attr.charisma), '</text><text x="250" y="300" class="title">Adventure</text><line x1="175" y1="305" x2="310" y2="305" style="stroke:#3e9aae;stroke-width:1"/></svg>'));
